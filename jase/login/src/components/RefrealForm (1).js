...
function CheckboxQuestions() {
  ...
  const handleEmploymentCheckboxChange = (event) => {
      const value = event.target.value;
      if (event.target.checked) {
          setEmploymentBarriers([...employmentBarriers, value]);
      } else {
          setEmploymentBarriers(employmentBarriers.filter(item => item !== value));
      }
  };

  const handleConvictionsCheckboxChange = (event) => {
      const value = event.target.value;
      if (event.target.checked) {
          setConvictions([...convictions, value]);
      } else {
          setConvictions(convictions.filter(item => item !== value));
      }
  };

  const handleSubmit = () => {
      console.log("Barriers related to employment:", employmentBarriers);
    ...
    return (
        <div className="form-container">
            ...
            <form>
                ...
                <div>
            <h2>Barriers related to employment</h2>
            <label><input type="checkbox" value="education" onChange={handleEmploymentCheckboxChange} /> Lack of relevant education or training</label><br />
            <label><input type="checkbox" value="experience" onChange={handleEmploymentCheckboxChange} /> Limited work experience</label><br />
            {/* Add more employment-related checkboxes as needed */}

            <h2>Convictions</h2>
            <label><input type="checkbox" value="felony" onChange={handleConvictionsCheckboxChange} /> Felony</label><br />
            <label><input type="checkbox" value="misdemeanor" onChange={handleConvictionsCheckboxChange} /> Misdemeanor</label><br />
            {/* Add more convictions checkboxes as needed */}
